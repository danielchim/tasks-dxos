...
export class Task extends dxos_echo_schema.TypedObject<TaskProps> {
  ...
  static filter(
    filter?: Partial<TaskProps> | dxos_echo_schema.OperatorFilter<Task>
  ): dxos_echo_schema.Filter<Task> {
    return dxos_echo_schema.Filter.typename("dxos.app.tasks.Task", filter);
  }
